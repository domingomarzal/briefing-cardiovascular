#!/usr/bin/env python3
# Fetch corpus N10: semana 2026/08/10 - 2026/08/16 (lun-dom), 31 revistas, con ArticleDate.
import urllib.request,urllib.parse,json,time,datetime,xml.etree.ElementTree as ET
EU="https://eutils.ncbi.nlm.nih.gov/entrez/eutils/"
D1,D2="2026/08/10","2026/08/16"
ES1,ES2="2026/08/08","2026/08/19"  # red amplia edat; la pertenencia la decide ArticleDate
...
for i in range(0,len(pmids),120):
    ...
    time.sleep(0.34)
json.dump(dict(periodo=f"{D1}-{D2}",recs=recs),open("n10_corpus.json","w"),ensure_ascii=False)
inwin=sum(1 for r in recs if D1<=r["adate"]<=D2)
print(f"semana {D1}-{D2}: {len(recs)} arts ({sum(1 for r in recs if r['abstract'])} con abstract); {inwin} con ArticleDate en ventana -> n10_corpus.json")
