#!/usr/bin/env python3
# Fetch corpus N12: semana 2026/08/24 - 2026/08/30 (SEMANA CONGRESO ESC) (lun-dom), 31 revistas, con ArticleDate.
import urllib.request,urllib.parse,json,time,datetime,xml.etree.ElementTree as ET
EU="https://eutils.ncbi.nlm.nih.gov/entrez/eutils/"
D1,D2="2026/08/24","2026/08/30"
ES1,ES2="2026/08/20","2026/09/03"  # red amplia edat; la pertenencia la decide ArticleDate
...
def esearch(term):
    try:
        return json.loads(get(EU+f"esearch.fcgi?db=pubmed&term={urllib.parse.quote(term)}&retmax=600&retmode=json"))["esearchresult"]["idlist"]
    except: return []
for j in CV:
    pmids.update(esearch(f'"{j}"[ta] AND {ES1}:{ES2}[edat]')); time.sleep(0.34)
for j in GENERAL:
    pmids.update(esearch(f'"{j}"[ta] AND {ES1}:{ES2}[edat] AND {THEME}')); time.sleep(0.34)
...
for i in range(0,len(pmids),120):
    ...
    time.sleep(0.34)
json.dump(dict(periodo=f"{D1}-{D2}",recs=recs),open("n12_corpus.json","w"),ensure_ascii=False)
inwin=sum(1 for r in recs if D1<=r["adate"]<=D2)
print(f"semana {D1}-{D2}: {len(recs)} arts ({sum(1 for r in recs if r['abstract'])} con abstract); {inwin} con ArticleDate en ventana -> n12_corpus.json")
