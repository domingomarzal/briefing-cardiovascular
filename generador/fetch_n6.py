#!/usr/bin/env python3
# Fetch corpus N6: semana 2026/07/13 - 2026/07/19 (lun-dom), 31 revistas, con ArticleDate.
...
for i in range(0,len(pmids),120):
    ...
    time.sleep(0.34)
json.dump(dict(periodo=f"{D1}-{D2}",recs=recs),open("n6_corpus.json","w"),ensure_ascii=False)
inwin=sum(1 for r in recs if D1<=r["adate"]<=D2)
print(f"semana {D1}-{D2}: {len(recs)} arts ({sum(1 for r in recs if r['abstract'])} con abstract); {inwin} con ArticleDate en ventana -> n6_corpus.json")
