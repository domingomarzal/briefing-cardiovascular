# -*- coding: utf-8 -*-
# Auditoría N11 (17 - 23 ago 2026). Parte del audit de N10 como plantilla y sustituye filas + datos.
...
corpus = json.load(open(GEN + "/n11_corpus.json"))["recs"]
sel = {s["pmid"]: s for s in json.load(open(GEN + "/n11_sel.json"))}
# lista de elegibles EXACTAMENTE como en n11_sel.py, para poder referirse a ellos por índice
EL = [r for r in corpus if D1 <= r['adate'] <= D2 and r['abstract'] and not (set(r['ptypes']) & BADT)]
def pm(*ix): return {EL[i]["pmid"] for i in ix}
...
# no cardiovascular: ictus/neurología pura y patología no CV colada por el filtro temático
NOCV = pm(4, 20, 27, 62, 101, 103, 105)
# ciencia básica / preclínica sin traslación clínica directa
BASICA = pm(0, 6, 7, 11, 12, 26, 31, 44, 45, 52, 70, 71, 76, 99, 102, 106, 120)
# ya publicado en un número anterior (PubMed le asigna nueva fecha online al entrar en el fascículo)
PREV = pm(18)
NOELIG = set()
rows = []
for a in corpus:
    ...
    if p in sel:
        ...
        star = "dest" if g["key"] == "a41" else ("top3" if g["key"] in ("a6", "a42", "a46") else "")
...
MOTTXT = {"top5": "fuera del top 5 de su sección", "basica": "ciencia básica / sin traslación clínica",
          ...
          "prev": "ya incluido en el N10 (mismo artículo, nueva fecha online)"}
def motcell(r):
...
box_html = (f'Revisados / seleccionados por revista (PubMed, 17–23 ago 2026; n.º revisados/seleccionados): {parts}. '
            f'<b>Total = {ntot} revisados · {nsel} seleccionados.</b>')
src = io.open(BASE + "/n10/articulos-revisados.html", encoding="utf-8").read()
sp = src.find("<tbody>") + len("<tbody>"); ep = src.rfind("</tbody>")
head = src[:sp]; tail = src[ep:]
head = head.replace("· Briefing Cardiovascular · N10", "· Briefing Cardiovascular · N11")
head = head.replace('<span class="num">N10</span>', '<span class="num">N11</span>')
head = head.replace("Artículos revisados · 10 al 16 de agosto de 2026", "Artículos revisados · 17 al 23 de agosto de 2026")
head = head.replace("2026/08/10–2026/08/16", "2026/08/17–2026/08/23")
head = head.replace("10–16 ago 2026", "17–23 ago 2026")
head = head.replace("Reglas de selección (N10).", "Reglas de selección (N11).")
...
NOTA_TOP3 = ('<p class="intro" style="margin-top:10px;"><b>Nota sobre el bloque «No te los puedes perder» (N11).</b> '
...
try:
    _fix = json.load(open(GEN + "/n11_linkfix.json"))
...
full = head + "\n" + rows_html + "\n" + tail
outs = [BASE + "/n11/articulos-revisados.html",
        "/Users/dmarzal/Documents/Claude/Briefing Cardiovascular/Briefing Cardiovascular_N11/Briefing Cardiovascular_N11_artículos revisados.html"]
for outp in outs:
    os.makedirs(os.path.dirname(outp), exist_ok=True)
    io.open(outp, "w", encoding="utf-8").write(full)
print("audit N11:", ntot, "filas,", nsel, "sel")
