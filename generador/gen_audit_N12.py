# -*- coding: utf-8 -*-
# Auditoría N12 (24 - 30 ago 2026, semana congreso ESC). Parte del audit de N10 como plantilla y sustituye filas + datos.
...
# antigua habría descartado (regla del usuario, 31/08/2026).
EL = json.load(open(GEN + "/n12_el.json"))
...
head = head.replace("· Briefing Cardiovascular · N10", "· Briefing Cardiovascular · N12")
head = head.replace('<span class="num">N10</span>', '<span class="num">N12</span>')
...
head = head.replace("Reglas de selección (N10).", "Reglas de selección (N12).")
...
NOTA_TOP3 = ('<p class="intro" style="margin-top:10px;"><b>Nota sobre el bloque «No te los puedes perder» (N12).</b> '
...
try:
    _fix = json.load(open(GEN + "/n12_linkfix.json"))
...
full = head + "\n" + rows_html + "\n" + tail
outs = [BASE + "/n12/articulos-revisados.html",
        "/Users/dmarzal/Documents/Claude/Briefing Cardiovascular/Briefing Cardiovascular_N12/Briefing Cardiovascular_N12_artículos revisados.html"]
for outp in outs:
    os.makedirs(os.path.dirname(outp), exist_ok=True)
    io.open(outp, "w", encoding="utf-8").write(full)
print("audit N12:", ntot, "filas,", nsel, "sel")
