# -*- coding: utf-8 -*-
# Auditoría N14 (7 - 13 sep 2026). Parte del audit de N13 como plantilla y sustituye filas + datos.
...
corpus = json.load(open(GEN + "/n14_corpus.json"))["recs"]
EL = json.load(open(GEN + "/n14_el.json"))
_selraw = json.load(open(GEN + "/n14_sel.json"))
...
# Motivos de descarte procedentes del CRIBADO (n14_out*.json), no de listas escritas a mano.
NOCV, BASICA, NOELIG, DUPN = set(), set(), set(), set()
for f in sorted(glob.glob(GEN + "/n14_out?.json")):
...
# Excluidos por DECISIÓN EDITORIAL (ámbito). En N14 no hay ninguno.
FUERA_AMBITO = set()
rows = []
for a in corpus:
    p = a["pmid"]
...
MOTTXT = {"top5": "fuera del top 5 de su sección", "basica": "ciencia básica / sin traslación clínica",
          ...
          "dup": "ya publicado en el número anterior (N13)",
...
box_html = (f'Revisados / seleccionados por revista (PubMed, 7 – 13 sep 2026; n.º revisados/seleccionados): {parts}. '
            f'<b>Total = {ntot} revisados · {nsel} seleccionados.</b>')
src = io.open(BASE + "/n13/articulos-revisados.html", encoding="utf-8").read()
sp = src.find("<tbody>") + len("<tbody>"); ep = src.rfind("</tbody>")
head = src[:sp]; tail = src[ep:]
head = head.replace("· Briefing Cardiovascular · N13", "· Briefing Cardiovascular · N14")
head = head.replace('<span class="num">N13</span>', '<span class="num">N14</span>')
...
head = re.sub(r'Artículos revisados · \d+[^<]*?de \d{4}',
              'Artículos revisados · 7 al 13 de septiembre de 2026', head)
head = re.sub(r'2026/\d\d/\d\d–2026/\d\d/\d\d', '2026/09/07–2026/09/13', head)
...
try:
    _fix = json.load(open(GEN + "/n14_linkfix.json"))
...
full = head + "\n" + rows_html + "\n" + tail
outs = [BASE + "/n14/articulos-revisados.html"]
_localdir = "/Users/dmarzal/Documents/Claude/Briefing Cardiovascular/Briefing Cardiovascular_N14"
if os.path.isdir(os.path.dirname(_localdir)):
    outs.append(_localdir + "/Briefing Cardiovascular_N14_artículos revisados.html")
for outp in outs:
    os.makedirs(os.path.dirname(outp), exist_ok=True)
    io.open(outp, "w", encoding="utf-8").write(full)
print("audit N14:", ntot, "filas,", nsel, "sel")
