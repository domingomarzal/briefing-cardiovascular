# -*- coding: utf-8 -*-
# Auditoría N15 (14 - 20 sep 2026). Parte del audit de N14 como plantilla y sustituye filas + datos.
...
corpus = json.load(open(GEN + "/n15_corpus.json"))["recs"]
EL = json.load(open(GEN + "/n15_el.json"))
_selraw = json.load(open(GEN + "/n15_sel.json"))
...
# Motivos de descarte procedentes del CRIBADO (n15_out*.json), no de listas escritas a mano.
NOCV, BASICA, NOELIG, DUPN = set(), set(), set(), set()
for f in sorted(glob.glob(GEN + "/n15_out?.json")):
    for o in json.load(open(f)):
        if o.get("elegible"): continue
        p = EL[o["idx"]]["pmid"]; m = o.get("motivo_descarte", "")
        if "duplicado" in m: DUPN.add(p)
        elif "no cardiovascular" in m: NOCV.add(p)
        elif "básica" in m: BASICA.add(p)
        else: NOELIG.add(p)
# Excluidos por DECISIÓN EDITORIAL (ámbito). En N15 no hay ninguno.
FUERA_AMBITO = set()
rows = []
for a in corpus:
    p = a["pmid"]
    ...
    rows.append(rec)
order_mot = {"": 0, "top5": 1, "ambito": 2, "dup": 2, "basica": 3, "nocv": 3, "tipo": 4, "periodo": 5}
...
MOTTXT = {"top5": "fuera del top 5 de su sección", "basica": "ciencia básica / sin traslación clínica",
          "ambito": "fuera del ámbito de la cardiología clínica",
          "dup": "ya publicado en el número anterior (N14)",
...
box_html = (f'Revisados / seleccionados por revista (PubMed, 14 – 20 sep 2026; n.º revisados/seleccionados): {parts}. '
            f'<b>Total = {ntot} revisados · {nsel} seleccionados.</b>')
src = io.open(BASE + "/n14/articulos-revisados.html", encoding="utf-8").read()
sp = src.find("<tbody>") + len("<tbody>"); ep = src.rfind("</tbody>")
head = src[:sp]; tail = src[ep:]
head = head.replace("· Briefing Cardiovascular · N14", "· Briefing Cardiovascular · N15")
head = head.replace('<span class="num">N14</span>', '<span class="num">N15</span>')
...
head = re.sub(r'Artículos revisados · \d+[^<]*?de \d{4}',
              'Artículos revisados · 14 al 20 de septiembre de 2026', head)
head = re.sub(r'2026/\d\d/\d\d–2026/\d\d/\d\d', '2026/09/14–2026/09/20', head)
head = re.sub(r'\d+\s*[–-]\s*\d+ (?:ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic) 2026',
              '14 – 20 sep 2026', head)
head = head.replace("Reglas de selección (N14).", "Reglas de selección (N15).")
NOTA_TOP3 = ('<p class="intro" style="margin-top:10px;"><b>Nota sobre el bloque «No te los puedes perder» (N15).</b> '
 'El <span class="star">★</span> <b>Destacado</b> y los tres titulares salen <b>directamente del ranking</b> de la rúbrica '
 'de 6 ejes, sin retoque editorial: a11 (7,91), a1 (7,31), a6 (6,80) y a16 (6,72). Semana sin congreso y sin guías '
 'europeas nuevas, de ahí que la puntuación máxima sea más baja de lo habitual. Los dos análisis cognitivos de '
 'CREST-2 (a1 y a2, JAMA) no los trajo la búsqueda temática de PubMed: se recuperaron en la auditoría de cobertura '
 'por Crossref (PASO 1b) y se ponderaron con la misma rúbrica.</p>')
...
# 1b) Y la frase gemela de la leyenda: en N14 los tres documentos de sociedad (a2, a39, a46)
...
try:
    _fix = json.load(open(GEN + "/n15_linkfix.json"))
...
full = head + "\n" + rows_html + "\n" + tail
outs = [BASE + "/n15/articulos-revisados.html"]
_localdir = "/Users/dmarzal/Documents/Claude/Briefing Cardiovascular/Briefing Cardiovascular_N15"
if os.path.isdir(os.path.dirname(_localdir)):
    outs.append(_localdir + "/Briefing Cardiovascular_N15_artículos revisados.html")
for outp in outs:
    os.makedirs(os.path.dirname(outp), exist_ok=True)
    io.open(outp, "w", encoding="utf-8").write(full)
print("audit N15:", ntot, "filas,", nsel, "sel")
