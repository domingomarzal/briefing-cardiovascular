# -*- coding: utf-8 -*-
# Auditoría N7 (20-26 jul 2026). Parte del audit de N6 como plantilla y sustituye filas + datos.
...
corpus = json.load(open(GEN + "/n7_corpus.json"))["recs"]
sel = {s["pmid"]: s for s in json.load(open(GEN + "/n7_sel.json"))}
...
NOCV = {"42492944",   # rasburicasa en síndrome de lisis tumoral (BMJ)
        "42500866"}   # recuperación motora en rehabilitación tras ictus
BASICA = {"42500876",  # PP4R2 y disfunción endotelial (ratón)
          "42473795",  # vesículas extracelulares y microglía en HFpEF (ratón)
          "42473784",  # S1PR2 de fibroblasto e inflamasoma NLRP3 (ratón)
          "42500885",  # progenitores cardiacos hiPSC en cerdo
          "42488951",  # linfocitos CD8+ autorreactivos frente a ApoB (mecanístico)
          "42478375",  # estructuras linfoides terciarias en aneurisma intracraneal
          "42475434",  # ATP-citrato liasa y disfunción del VI (ratón)
          "42500870",  # hipotermia y células madre en modelo neonatal murino
          "42493558",  # nucleoma 4D cardiaco (revisión mecanística)
          "42492123",  # pro-N-cadherina en primates no humanos irradiados
          "42500881",  # empagliflozina en remodelado vascular pulmonar experimental
          "42476389",  # comportamiento térmico de la ablación por campo pulsado (banco/animal)
          "42500892",  # multiómica sérica en enfermedad de moyamoya
          "42500868",  # multiómica y dianas terapéuticas en aneurisma de aorta torácica
          "42483810"}  # Donald Seldin Lecture: genómica poblacional y multiómica (revisión)
NOELIG = {"42473047",  # "expert viewpoint" sobre estimulación del sistema de conducción
          "42473040"}  # reporte breve de perforación septal en LBBAP
rows = []
for a in corpus:
    p = a["pmid"]
    inwin = D1 <= a["adate"] <= D2
    ...
    if p in sel:
        ...
        star = "dest" if g["key"] == "a11" else ("top3" if g["key"] in ("a51", "a46", "a6") else "")
...
box_html = (f'Revisados / seleccionados por revista (PubMed, 20–26 jul 2026; n.º revisados/seleccionados): {parts}. '
            f'<b>Total = {ntot} revisados · {nsel} seleccionados.</b>')
src = io.open(BASE + "/n6/articulos-revisados.html", encoding="utf-8").read()
sp = src.find("<tbody>") + len("<tbody>"); ep = src.rfind("</tbody>")
head = src[:sp]; tail = src[ep:]
head = head.replace("· Briefing Cardiovascular · N6", "· Briefing Cardiovascular · N7")
head = head.replace('<span class="num">N6</span>', '<span class="num">N7</span>')
head = head.replace("Artículos revisados · 13 al 19 de julio de 2026", "Artículos revisados · 20 al 26 de julio de 2026")
head = head.replace("2026/07/13–2026/07/19", "2026/07/20–2026/07/26")
head = head.replace("13–19 jul 2026", "20–26 jul 2026")
head = head.replace("Reglas de selección (N6).", "Reglas de selección (N7).")
...
full = head + "\n" + rows_html + "\n" + tail
outs = [BASE + "/n7/articulos-revisados.html",
        "/Users/dmarzal/Documents/Claude/Briefing Cardiovascular/Briefing Cardiovascular_N7/Briefing Cardiovascular_N7_artículos revisados.html"]
for outp in outs:
    os.makedirs(os.path.dirname(outp), exist_ok=True)
    io.open(outp, "w", encoding="utf-8").write(full)
print("audit N7:", ntot, "filas,", nsel, "sel")
