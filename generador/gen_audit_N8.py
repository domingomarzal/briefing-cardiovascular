# -*- coding: utf-8 -*-
# Auditoría N8 (27 jul - 2 ago 2026). Parte del audit de N7 como plantilla y sustituye filas + datos.
...
corpus = json.load(open(GEN + "/n8_corpus.json"))["recs"]
sel = {s["pmid"]: s for s in json.load(open(GEN + "/n8_sel.json"))}
...
NOCV = {"42521817"}   # arquitectura genética de la fibromialgia (Nat Med)
BASICA = {"42535523",  # dolor crónico + dieta rica en grasa en ratón ApoE-/-
          "42517561",  # bucle AMPK/USP10 y ACE2 en hipertensión pulmonar (in silico/in vitro)
          "42522575",  # sobrecarga de metilmalonato en el corazón diabético (ratón)
          "42504620",  # denervación renal y neuroinflamación hipotalámica (rata)
          "42531358",  # proteómica de célula única y multiómica espacial (revisión mecanística)
          "42541992",  # lactato y formación de células espumosas (ratón)
          "42531359",  # desarrollo y maduración cardiaca a resolución de célula única (revisión)
          "42535557",  # lipina-1 en el macrófago de la placa (revisión mecanística)
          "42535988",  # marcapasos biológico TBX18 en modelo porcino
          "42504621",  # omaveloxolona y activación de NRF2 en hipertensión pulmonar (preclínico)
          "42538380",  # transcriptómica espaciotemporal de progenitores humanos en cerdo
          "42517592",  # endofilina A2 y degradación de eNOS (preclínico)
          "42531357",  # proteómica espacial en enfermedad cardiovascular (revisión)
          "42522548",  # sTREM2 e inflamación mieloide en aterosclerosis (ratón)
          "42535536",  # trombosis del stent carotídeo en modelo macrofluídico
          "42535577"}  # CRABP2 y activación del fibroblasto cardiaco (ratón)
NOELIG = set()
rows = []
for a in corpus:
    p = a["pmid"]
    ...
    if p in sel:
        ...
        star = "dest" if g["key"] == "a6" else ("top3" if g["key"] in ("a20", "a45", "a11") else "")
...
box_html = (f'Revisados / seleccionados por revista (PubMed, 27 jul–2 ago 2026; n.º revisados/seleccionados): {parts}. '
            f'<b>Total = {ntot} revisados · {nsel} seleccionados.</b>')
src = io.open(BASE + "/n7/articulos-revisados.html", encoding="utf-8").read()
sp = src.find("<tbody>") + len("<tbody>"); ep = src.rfind("</tbody>")
head = src[:sp]; tail = src[ep:]
head = head.replace("· Briefing Cardiovascular · N7", "· Briefing Cardiovascular · N8")
head = head.replace('<span class="num">N7</span>', '<span class="num">N8</span>')
head = head.replace("Artículos revisados · 20 al 26 de julio de 2026", "Artículos revisados · 27 de julio al 2 de agosto de 2026")
head = head.replace("2026/07/20–2026/07/26", "2026/07/27–2026/08/02")
head = head.replace("20–26 jul 2026", "27 jul–2 ago 2026")
head = head.replace("Reglas de selección (N7).", "Reglas de selección (N8).")
...
try:
    _fix = json.load(open(GEN + "/n8_linkfix.json"))
...
full = head + "\n" + rows_html + "\n" + tail
outs = [BASE + "/n8/articulos-revisados.html",
        "/Users/dmarzal/Documents/Claude/Briefing Cardiovascular/Briefing Cardiovascular_N8/Briefing Cardiovascular_N8_artículos revisados.html"]
for outp in outs:
    os.makedirs(os.path.dirname(outp), exist_ok=True)
    io.open(outp, "w", encoding="utf-8").write(full)
print("audit N8:", ntot, "filas,", nsel, "sel")
