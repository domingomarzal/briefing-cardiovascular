...
(function(){
 ...
 (function(){
  ...
  document.querySelectorAll('.lang button').forEach(function(x){ x.addEventListener('click', function(){ voz = null; parar(); }); });
  ...
  function montaVoces(){
    if (!bar || bar.querySelector('select')) return;
    var vs = voces(); if (vs.length < 2) return;
  ...
  if (SS.onvoiceschanged !== undefined) SS.addEventListener && SS.addEventListener('voiceschanged', montaVoces);
 })();
