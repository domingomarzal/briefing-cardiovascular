...
.gesto button{pointer-events:auto;display:inline-flex;align-items:center;gap:10px;
...
(function(){
 ...
 (function(){
  ...
  /* Velocidad: por defecto 1,25x en español y 1x en inglés — las voces
     españolas suenan lentas al ritmo neutro. La elección manual se recuerda
     POR NÚMERO y por idioma: cada briefing nuevo estrena el valor por defecto,
     y volver a uno ya escuchado recupera lo que ajustaste allí. */
  var vel = 1;
  ...
  document.querySelectorAll('.lang button').forEach(function(x){
    x.addEventListener('click', function(){
      voz = null; parar();
      setTimeout(function(){ montaVoces(); montaVel(); }, 60);         /* voz y ritmo del idioma nuevo */
    });
  });
  ...
  function montaVel(){
    ...
    if (sv.dataset.listo) return;                    /* el listener se pone una sola vez */
    sv.dataset.listo = '1';
    sv.addEventListener('change', function(){
      vel = parseFloat(sv.value) || velDef();
      try { localStorage.setItem(velKey(), String(vel)); } catch(e){}
  ...
  /* Si venimos de "Escuchar en Chrome", el hash trae qué había que escuchar.
     ...
     más cargar se descarta EN SILENCIO. Por eso lo intentamos, comprobamos si
     ha sonado de verdad y, si no, ofrecemos un botón grande — un solo toque. */
  (function retoma(){
    var h = location.hash || '';
    var m = h.match(/audio=([a-z0-9\-]+)/i); if (!m) return;
    var l = (h.match(/lang=(es|en)/i) || [])[1];
    history.replaceState(null, '', location.pathname + location.search);
    /* Venir con este hash significa que el usuario YA eligió Chrome: si no lo
       marcamos, el diálogo se vuelve a abrir aquí, tapa la página y el audio
       nunca llega a sonar. */
    try { sessionStorage.setItem('brief-chrome-ok', '1'); } catch(e){}
    ...
    function lanza(){
      ...
      var r = m[1], b = null;
      if (r === 'all') b = document.querySelector('.mast .abtn');
      else if (r === 'top3') b = document.querySelector('.abtn[data-a="top3"]');
      else if (r.indexOf('sec-') === 0) b = document.querySelector('.abtn[data-a="sec"][data-ref="' + r.slice(4) + '"]');
...
CONFIGS = [
 ...
 dict(n="n15", linkfix=_acr(BASE+"/generador/n15_linkfix.json"), jlinks=_acr(BASE+"/generador/n15_jlinks.json"), data=BASE+"/generador/n15_data.json", num="Nº 15", period=("14 al 20 de septiembre de 2026","September 14–20, 2026"), dest="a55", top3=["a57","a6","a32"], acr=_acr(BASE+"/generador/n15_acr.json"), viz=_viz(BASE+"/generador/n15_viz.html"), local="Briefing Cardiovascular_N15", lnum="N15"),
]
import sys as _sys
