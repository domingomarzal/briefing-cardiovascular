#!/usr/bin/env python3
# Genera el cuerpo (texto plano + htmlBody) del borrador de Gmail "Cardio al día_N10".
import json, os, html
...
htmlbody = (
 ...
 f'<div style="font-size:13px;color:#c4d4df;margin-top:5px;">N10 &middot; {PERIOD}</div></a></td>'
...
plain = (f"Cardio al día · N10 · {PERIOD}\n{URL}\n\n"
         f"DESTACADO DE LA SEMANA\n{t(DEST)}\n{D[DEST]['journal']}: {doi(DEST)}\n\n"
         f"NO TE LOS PUEDES PERDER\n")
for i, k in enumerate(TOP3, 1):
    plain += f"{i}. {t(k)}\n   {D[k]['journal']}: {doi(k)}\n"

json.dump(dict(subject="Cardio al día_N10", to="domingo.marzal@gmail.com",
               body=plain, htmlBody=htmlbody),
          open(G + "/n10_draft.json", "w"), ensure_ascii=False, indent=1)
print(plain)
print("--- htmlBody:", len(htmlbody), "chars -> n10_draft.json")
