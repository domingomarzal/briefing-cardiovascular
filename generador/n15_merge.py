#!/usr/bin/env python3
# Fusiona metadatos (n15_sel.json) + textos de los 5 lotes (n15_dataN.json) -> n15_data.json
import json, os, glob
B = os.path.dirname(os.path.abspath(__file__))
SEL = {o['key']: o for o in json.load(open(B+'/n15_sel.json'))}
TXT = {}
for f in sorted(glob.glob(B+'/n15_data[0-4].json')):
...
json.dump(out, open(B+'/n15_data.json','w'), ensure_ascii=False, indent=1)
print(f"n15_data.json: {len(out)} fichas")
