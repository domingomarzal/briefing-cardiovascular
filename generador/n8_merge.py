#!/usr/bin/env python3
# Fusiona n8_sel.json + n8_out{0..4}.json -> n8_data.json (formato que consume gen_bilingue.py)
import json, os
G = os.path.dirname(os.path.abspath(__file__))
sel = {s["key"]: s for s in json.load(open(G + "/n8_sel.json"))}
fich = {}
for b in range(5):
    for o in json.load(open(f"{G}/n8_out{b}.json")):
        fich[o["key"]] = o
data = {}
...
json.dump(data, open(G + "/n8_data.json", "w"), ensure_ascii=False, indent=1)
from collections import Counter
print("n8_data.json:", len(data), "fichas ·", dict(sorted(Counter(a["sec"] for a in data.values()).items())))
