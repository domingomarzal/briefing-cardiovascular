#!/usr/bin/env python3
# Genera el cuerpo (texto plano + htmlBody) del borrador de Gmail "Cardio al día_N9".
import json, os, html
G = os.path.dirname(os.path.abspath(__file__))
D = json.load(open(G + "/n9_data.json"))
ACR = json.load(open(G + "/n9_acr.json"))
DEST = "a43"; TOP3 = ["a6", "a18", "a44"]
PERIOD = "3 al 9 de agosto de 2026"
URL = "https://domingomarzal.github.io/briefing-cardiovascular/n9/cardio-al-dia.html"
...
htmlbody = (
 ...
 f'<div style="font-size:13px;color:#c4d4df;margin-top:5px;">N9 &middot; {PERIOD}</div></a></td>'
...
plain = (f"Cardio al día · N9 · {PERIOD}\n{URL}\n\n"
         f"DESTACADO DE LA SEMANA\n{t(DEST)}\n{D[DEST]['journal']}: {doi(DEST)}\n\n"
         f"NO TE LOS PUEDES PERDER\n")
for i, k in enumerate(TOP3, 1):
    plain += f"{i}. {t(k)}\n   {D[k]['journal']}: {doi(k)}\n"

json.dump(dict(subject="Cardio al día_N9", to="domingo.marzal@gmail.com",
               body=plain, htmlBody=htmlbody),
          open(G + "/n9_draft.json", "w"), ensure_ascii=False, indent=1)
print(plain)
print("--- htmlBody:", len(htmlbody), "chars -> n9_draft.json")
