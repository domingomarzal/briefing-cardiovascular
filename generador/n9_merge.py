#!/usr/bin/env python3
# Fusiona n9_sel.json + n9_out{0..4}.json -> n9_data.json (formato que consume gen_bilingue.py)
import json, os
G = os.path.dirname(os.path.abspath(__file__))
sel = {s["key"]: s for s in json.load(open(G + "/n9_sel.json"))}
fich = {}
for b in range(5):
    for o in json.load(open(f"{G}/n9_out{b}.json")):
        fich[o["key"]] = o
data = {}
...
json.dump(data, open(G + "/n9_data.json", "w"), ensure_ascii=False, indent=1)
from collections import Counter
print("n9_data.json:", len(data), "fichas ·", dict(sorted(Counter(a["sec"] for a in data.values()).items())))
